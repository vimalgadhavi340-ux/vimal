...
export const ImageGenerator: React.FC<ImageGeneratorProps> = ({ initialUserName = "Pro Creator" }) => {
  ...
  const handleFeedback = (id: string, type: 'up' | 'down') => {
    setHistory(prev => prev.map(img => {
        if (img.id === id) {
            return {
                ...img,
                feedback: img.feedback === type ? undefined : type
            };
        }
        return img;
    }));
    ...
    if (currentImage && currentImage.id === id) {
        setCurrentImage(prev => {
             ...
             return {
                 ...prev,
                 feedback: prev.feedback === type ? undefined : type
             };
        });
    }
  };
    ...
    <div className="space-y-1.5 w-full">
        ...
        <div className="relative group">
            <select 
                ...
                disabled={rawMode}
                className={`w-full appearance-none bg-[#0a0a0a] border rounded-lg px-3 py-2 text-[11px] font-semibold text-gray-300 focus:outline-none focus:ring-1 focus:ring-indigo-500/50 focus:border-indigo-500/50 cursor-pointer h-[38px] transition-all ${rawMode ? 'border-white/5 opacity-40 cursor-not-allowed' : 'border-white/10 group-hover:border-white/20'}`}
            >
                {options.map(opt => <option key={opt.id} value={opt.id}>{opt.label}</option>)}
            </select>
            <div className={`absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none transition-colors ${rawMode ? 'text-gray-700' : 'text-gray-600 group-hover:text-gray-400'}`}>
      ...
      return (
        <div className={`${sizeClass} rounded-full overflow-hidden flex-shrink-0 flex items-center justify-center border transition-all relative ${isSelected ? 'border-indigo-500 scale-110 shadow-[0_0_10px_rgba(99,102,241,0.5)]' : 'border-white/10 shadow-sm'}`} style={{ background: !isImage ? avatar : undefined }}>
            {isImage ? (
                ...
                <IconUser className={`${iconSizeClass} text-white`} />
            )}
             {isSelected && (
  ...
  return (
    <div className="h-[100dvh] w-full flex flex-col md:flex-row bg-[#050505] text-white overflow-hidden font-sans selection:bg-indigo-500/30 selection:text-white">
      ...
      <div className="hidden md:flex flex-col w-80 border-r border-white/5 bg-[#0a0a0a] h-full">
        <div className="p-8 pb-4">
          <div className="flex items-center gap-2 mb-8">
             ...
             <span className="font-display font-bold text-xl tracking-tight">Krati.ai</span>
          </div>
          
          <button 
            onClick={handleNewProject}
            className="w-full py-3 rounded-xl bg-white/5 hover:bg-white/10 border border-white/5 text-sm font-medium transition-all flex items-center justify-center gap-2 group"
          >
        ...
        <div className="flex-1 overflow-y-auto px-4 py-2 space-y-1 custom-scrollbar">
            <h3 className="px-4 text-[11px] font-semibold text-gray-500 uppercase tracking-wider mb-2 mt-2">Library</h3>
            {history.length === 0 ? (
                <div className="px-4 py-8 text-center">
                    <div className="w-12 h-12 rounded-full bg-white/5 mx-auto flex items-center justify-center mb-3">
                        <IconImage className="w-5 h-5 text-gray-600" />
                    </div>
                ...
                history.map(img => (
                    <button 
                        ...
                        className={`w-full text-left group p-2 rounded-xl flex items-center gap-3 transition-all ${currentImage?.id === img.id ? 'bg-[#18181b] shadow-sm' : 'hover:bg-white/5'}`}
                    >
                        <div className="w-12 h-12 rounded-lg overflow-hidden bg-black flex-shrink-0 border border-white/10">
                            <img src={img.url} alt="thumbnail" className="w-full h-full object-cover" />
                        </div>
                        <div className="flex-1 min-w-0">
                            <p className={`text-xs font-medium truncate ${currentImage?.id === img.id ? 'text-white' : 'text-gray-400 group-hover:text-gray-200'}`}>{img.prompt}</p>
                            <p className="text-[10px] text-gray-600 mt-0.5">{new Date(img.timestamp).toLocaleTimeString()}</p>
                        </div>
                    </button>
                ))
            )}
        ...
        <div className="p-4 border-t border-white/5 bg-[#0a0a0a]">
             <div 
              ...
              >
                 {renderAvatarCircle(userAvatar, "w-9 h-9")}
                 <div className="flex-1 min-w-0">
                     <p className="text-sm font-semibold text-gray-200 group-hover:text-white truncate">{userName}</p>
                     <p className="text-[10px] text-indigo-400 font-medium">Pro Plan Active</p>
                 </div>
                 <IconPen className="w-3.5 h-3.5 text-gray-600 group-hover:text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity" />
             </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in p-4">
           <div className="bg-[#18181b] border border-white/10 rounded-2xl w-full max-w-sm p-6 shadow-2xl scale-100 animate-in zoom-in-95">
              ...
              <div className="space-y-6">
                 <div>
                    <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3 block">Choose Avatar</label>
                    <div className="grid grid-cols-5 gap-3 justify-center">
                       {AVATARS.map((bg, idx) => (
                          <button 
                            key={idx}
                            onClick={() => setUserAvatar(bg)}
                 ...
                 <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-[#0e0e0e] border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-indigo-500 transition-colors"
                    />
                 </div>
                 ...
                 <button 
                    ...
                    className="w-full py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold transition-colors shadow-lg shadow-indigo-500/20"
                 >
                    Save Changes
                 </button>
              </div>
      ...
      <div className="flex-1 flex flex-col h-full relative bg-[#050505] overflow-y-auto scroll-smooth">
        ...
        <div className="md:hidden flex items-center justify-between p-4 bg-[#050505]/80 backdrop-blur-md sticky top-0 z-20 border-b border-white/5 shrink-0">
            ...
            <div className="flex items-center gap-2">
                 <button onClick={() => setShowProfileModal(true)} className="md:hidden">
                    {renderAvatarCircle(userAvatar, "w-8 h-8")}
                 </button>
                <button 
                    onClick={handleNewProject}
        ...
        <div className="flex-1 flex flex-col items-center p-4 md:p-8 max-w-6xl mx-auto w-full gap-8 pb-32">
            ...
            <div className="w-full max-w-4xl space-y-6">
                ...
                <div className="flex justify-center">
                    <div className="bg-[#121212] p-1 rounded-full border border-white/10 inline-flex">
                        <button
                            onClick={() => setMode('text')}
                            className={`px-6 py-2.5 rounded-full text-xs font-semibold transition-all flex items-center gap-2 ${mode === 'text' ? 'bg-[#27272a] text-white shadow-lg' : 'text-gray-500 hover:text-gray-300'}`}
                        >
                        ...
                        <button
                            ...
                            className={`px-6 py-2.5 rounded-full text-xs font-semibold transition-all flex items-center gap-2 ${mode === 'style' ? 'bg-[#27272a] text-white shadow-lg' : 'text-gray-500 hover:text-gray-300'}`}
                        >
                    ...
                    <div className="bg-[#0e0e0e] rounded-[22px] p-6 md:p-8 space-y-6 relative z-10">
                        ...
                        {mode === 'style' && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-3">
                                    <label className="text-xs font-semibold text-gray-400 ml-1">Structure Reference</label>
                                    <div 
                                        className={`relative group aspect-video rounded-2xl border-2 border-dashed transition-all cursor-pointer overflow-hidden ${contentImg ? 'border-indigo-500/30 bg-indigo-500/5' : 'border-[#27272a] hover:border-gray-600 bg-[#121212]'}`}
                                        onClick={() => !contentImg && contentInputRef.current?.click()}
                                    >
                                        ...
                                        {contentImg ? (
                                            <>
                                                ...
                                                <button onClick={(e) => { e.stopPropagation(); setContentImg(null); }} className="absolute top-2 right-2 p-2 bg-black/60 rounded-full text-white hover:bg-black/80 transition-colors"><IconX className="w-4 h-4" /></button>
                                            </>
                                        ) : (
                                            <div className="flex flex-col items-center justify-center h-full text-gray-600 gap-2"><IconImage className="w-8 h-8 opacity-50" /><span className="text-xs">Upload Structure</span></div>
                                        )}
                                    </div>
                                </div>
                                <div className="space-y-3">
                                    <label className="text-xs font-semibold text-gray-400 ml-1">Style Reference</label>
                                    <div 
                                        className={`relative group aspect-video rounded-2xl border-2 border-dashed transition-all cursor-pointer overflow-hidden ${styleImg ? 'border-fuchsia-500/30 bg-fuchsia-500/5' : 'border-[#27272a] hover:border-gray-600 bg-[#121212]'}`}
                                        onClick={() => !styleImg && styleInputRef.current?.click()}
                                    >
                                        ...
                                        {styleImg ? (
                                            <>
                                                ...
                                                <button onClick={(e) => { e.stopPropagation(); setStyleImg(null); }} className="absolute top-2 right-2 p-2 bg-black/60 rounded-full text-white hover:bg-black/80 transition-colors"><IconX className="w-4 h-4" /></button>
                                            </>
                                        ) : (
                                            <div className="flex flex-col items-center justify-center h-full text-gray-600 gap-2"><IconUpload className="w-8 h-8 opacity-50" /><span className="text-xs">Upload Style</span></div>
                                        )}
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="relative bg-[#18181b] rounded-2xl border border-white/5 focus-within:border-indigo-500/50 transition-colors p-4">
                            <textarea 
                                ...
                                placeholder={mode === 'text' ? "Describe your imagination..." : "Add specific details (optional)..."}
                                className="w-full bg-transparent border-0 focus:ring-0 text-lg text-white placeholder-gray-600 resize-none min-h-[80px] leading-relaxed"
                                rows={2}
                            />
                            ...
                            {mode === 'text' && referenceImg && (
                                <div className="mt-3 flex items-start gap-3 p-2 bg-white/5 rounded-xl border border-white/10 w-fit">
                                     <div className="w-16 h-16 rounded-lg overflow-hidden bg-black flex-shrink-0 relative">
                                        <img src={referenceImg.previewUrl} className="w-full h-full object-cover opacity-80" alt="ref" />
                                     </div>
                                     <div className="flex flex-col h-16 justify-between py-0.5">
                                        <span className="text-[10px] text-gray-400 font-semibold uppercase tracking-wider">Reference Image</span>
                                        <button onClick={() => setReferenceImg(null)} className="text-[10px] text-red-400 hover:text-red-300 font-medium text-left">Remove</button>
                                     </div>
                                </div>
                            )}
                            ...
                            <div className="flex items-center justify-between mt-2 pt-2 border-t border-white/5">
                                <div className="flex gap-2">
                                     {mode === 'text' && (
                                        <>
                                            ...
                                            <button
                                                ...
                                                className={`text-xs font-medium transition-colors flex items-center gap-1.5 px-2 py-1 rounded-lg hover:bg-white/5 ${referenceImg ? 'text-indigo-400' : 'text-gray-500 hover:text-indigo-400'}`}
                                     ...
                                     <button
                                        ...
                                        className="text-xs font-medium text-gray-500 hover:text-indigo-400 transition-colors flex items-center gap-1.5 px-2 py-1 rounded-lg hover:bg-white/5"
                                    >
                                    ...
                                    <button
                                        ...
                                        className={`text-xs font-medium transition-colors flex items-center gap-1.5 px-2 py-1 rounded-lg hover:bg-white/5 ${showAdvanced ? 'text-indigo-400 bg-white/5' : 'text-gray-500 hover:text-indigo-400'}`}
                                    >
                                ...
                                {mode === 'text' && (
                                    <button
                                        ...
                                        className="text-xs font-medium gradient-text transition-opacity flex items-center gap-1.5 px-2 py-1 rounded-lg hover:bg-white/5 disabled:opacity-50"
                                    >
                            ...
                            {showSuggestions && (
                                <div className="absolute top-full left-0 right-0 mt-2 bg-[#18181b] border border-white/10 rounded-2xl p-4 z-20 shadow-xl shadow-black/50 animate-in fade-in slide-in-from-top-2">
                                    <div className="flex items-center justify-between mb-3">
                                        <span className="text-xs font-semibold text-gray-400">Inspiration</span>
                                        <button onClick={() => setShowSuggestions(false)}><IconX className="w-4 h-4 text-gray-500 hover:text-white" /></button>
                                    </div>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-60 overflow-y-auto">
                                        {PROMPT_SUGGESTIONS.map((category) => (
                                            <div key={category.category}>
                                                <h4 className="text-[10px] font-bold text-indigo-400 mb-1.5 uppercase">{category.category}</h4>
                                                <div className="space-y-1">
                                                    {category.prompts.map((p, idx) => (
                                                        <button key={idx} onClick={() => handleSuggestionSelect(p)} className="w-full text-left text-[11px] p-2 rounded-lg hover:bg-white/5 text-gray-300 transition-colors line-clamp-2">{p}</button>
                                                    ))}
                                                </div>
                                            </div>
                                        ))}
                        ...
                        {mode === 'text' && (
                            <div className={`bg-[#18181b]/50 rounded-xl border border-white/5 p-4 animate-in fade-in slide-in-from-top-2 backdrop-blur-sm transition-all duration-300 ${rawMode ? 'opacity-50 grayscale pointer-events-none' : 'opacity-100'}`}>
                                <div className="flex items-center justify-between mb-3">
                                    <label className="text-[11px] font-bold text-indigo-400 uppercase tracking-wider flex items-center gap-2">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="bg-[#18181b] p-4 rounded-xl border border-white/5">
                                        <div className="flex items-center justify-between mb-3">
                                            ...
                                            <span className="text-xs text-indigo-400 font-mono">{Math.round(creativity * 100)}%</span>
                                        </div>
                                        <input 
                                            type="range" 
                                            min="0" 
                                    ...
                                    <div className="bg-[#18181b] p-4 rounded-xl border border-white/5 flex items-center justify-between">
                                        <div>
                                            ...
                                            <p className="text-[10px] text-gray-500 mt-0.5">Bypass all styles for exact prompt adherence</p>
                                        </div>
                                        <button 
                                            onClick={() => setRawMode(!rawMode)}
                                            className={`w-12 h-6 rounded-full p-1 transition-colors duration-300 relative ${rawMode ? 'bg-indigo-600' : 'bg-gray-700'}`}
                                        >
                                            <div className={`w-4 h-4 bg-white rounded-full shadow-md transform transition-transform duration-300 ${rawMode ? 'translate-x-6' : 'translate-x-0'}`}></div>
                                        </button>
                                    </div>
                                </div>

                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div className="md:col-span-2 bg-[#18181b] p-4 rounded-xl border border-white/5">
                                        <label className="text-xs font-medium text-gray-400 block mb-2">Negative Prompt (Exclude)</label>
                                        <input 
                                            type="text" 
                                            value={negativePrompt}
                                            onChange={(e) => setNegativePrompt(e.target.value)}
                                            placeholder="blur, distortion, low quality, watermark..."
                                            className="w-full bg-[#0e0e0e] border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:border-indigo-500/50 outline-none"
                                        />
                                    </div>
                                    <div className="bg-[#18181b] p-4 rounded-xl border border-white/5">
                                        <div className="flex items-center justify-between mb-2">
                                            <label className="text-xs font-medium text-gray-400 flex items-center gap-1.5">
                                                <IconDice className="w-3.5 h-3.5" /> Seed
                                            </label>
                                            <button onClick={() => setSeed(Math.floor(Math.random() * 999999999))} className="text-[10px] text-indigo-400 hover:text-indigo-300">Randomize</button>
                                        </div>
                                        <input 
                                            type="number" 
                                            value={seed ?? ''}
                        ...
                        <div className="flex flex-col md:flex-row gap-6 items-end md:items-center pt-2">
                                        ...
                                        <div className="flex flex-wrap gap-2 bg-[#18181b] p-1.5 rounded-xl border border-white/5">
                                            {ASPECT_RATIOS.map(ratio => (
                                                <button
                                                    ...
                                                    title={ratio.label}
                                                >
                                                    {renderRatioIcon(ratio.icon, "w-3.5 h-3.5")}
                                        ...
                                        <div className="flex bg-[#18181b] p-1.5 rounded-xl border border-white/5 h-full items-center">
                                            {RESOLUTIONS.map(res => (
                                                <button
                                                    ...
                                                    className={`px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all ${resolution === res ? 'bg-[#27272a] text-white shadow-sm ring-1 ring-white/10' : 'text-gray-500 hover:text-gray-300 hover:bg-white/5'}`}
                                                >
                                                    {res}
                                                </button>
                                            ))}
                            ...
                            <button
                                ...
                                className={`w-full md:w-auto min-w-[160px] h-[52px] rounded-full font-bold text-sm tracking-wide transition-all duration-300 transform active:scale-95 flex items-center justify-center gap-2 shadow-lg shadow-indigo-900/20
                                    ${isGenerating 
                                        ? 'bg-[#18181b] border border-white/10 cursor-default' 
                                        : (mode === 'text' && !prompt.trim() && !referenceImg) || (mode === 'style' && (!contentImg || !styleImg)) 
                                            ? 'bg-[#18181b] text-gray-600 border border-white/5 cursor-not-allowed'
                                            : 'gradient-bg text-white hover:brightness-110'
                                    }`}
                            >
                                {isGenerating ? (
                                    <div className="flex flex-col items-center w-full px-4">
                                        <div className="flex items-center justify-between w-full text-[10px] text-gray-400 mb-1">
                                            <span>Generating</span>
                                            <span>{Math.round(progress)}%</span>
                                        </div>
                                        <div className="w-full h-1 bg-gray-800 rounded-full overflow-hidden">
                                            <div className="h-full gradient-bg transition-all duration-300 ease-out" style={{ width: `${progress}%` }}></div>
                                        </div>
                                    </div>
                                ) : (
                                    <>
                ...
                {currentImage ? (
                    <div className="relative group w-full h-full flex items-center justify-center p-4">
                        ...
                        <div className={`relative z-10 overflow-hidden shadow-2xl rounded-2xl ring-1 ring-white/10 transition-all duration-700 ease-out animate-in fade-in zoom-in-95
                            ${currentImage.aspectRatio === '1:1' ? 'aspect-square max-h-[70vh]' : 
                              currentImage.aspectRatio === '16:9' ? 'aspect-video max-w-full' : 
                              currentImage.aspectRatio === '9:16' ? 'aspect-[9/16] max-h-[70vh]' : 'aspect-square max-h-[70vh]'
                            }
                        `}>
                            ...
                            <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-6">
                                <div className="transform translate-y-4 group-hover:translate-y-0 transition-transform">
                                    <p className="text-white/90 font-medium text-sm line-clamp-2 mb-4 drop-shadow-md">{currentImage.prompt}</p>
                                    <div className="flex items-center justify-between gap-3">
                                        <div className="flex flex-1 gap-2">
                                            <button 
                                                onClick={() => handleDownload(currentImage)}
                                                className="flex-1 inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-white text-black rounded-full text-xs font-bold hover:bg-gray-200 transition-colors shadow-lg"
                                            >
                                                <IconDownload className="w-4 h-4" />
                                                Download {currentImage.resolution}
                                            </button>
                                            <button
                                                onClick={() => handleRemix(currentImage)}
                                                className="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-white/10 backdrop-blur-md text-white rounded-full text-xs font-bold hover:bg-white/20 transition-colors border border-white/10"
                                        ...
                                        <div className="flex items-center gap-2">
                                            <button onClick={() => handleFeedback(currentImage.id, 'up')} className={`w-9 h-9 rounded-full flex items-center justify-center backdrop-blur-md border transition-all ${currentImage.feedback === 'up' ? 'bg-green-500/20 border-green-500/50 text-green-400' : 'bg-black/40 border-white/10 text-white hover:bg-black/60'}`}>
                                                <IconThumbUp className="w-4 h-4" />
                                            </button>
                                            <button onClick={() => handleFeedback(currentImage.id, 'down')} className={`w-9 h-9 rounded-full flex items-center justify-center backdrop-blur-md border transition-all ${currentImage.feedback === 'down' ? 'bg-red-500/20 border-red-500/50 text-red-400' : 'bg-black/40 border-white/10 text-white hover:bg-black/60'}`}>
                    ...
                    <div className="text-center text-gray-700 space-y-6 opacity-60">
                         <div className="relative">
                            <div className="absolute inset-0 bg-indigo-500/20 blur-3xl rounded-full"></div>
                            <div className="relative w-24 h-24 mx-auto border border-white/10 rounded-2xl flex items-center justify-center bg-[#0a0a0a]">
                                <IconLandscape className="w-10 h-10 text-gray-600" />
                            </div>
                         </div>
                        <p className="text-sm font-medium tracking-wide">Ready to Create</p>
                    </div>
                )}
            </div>
        </div>
